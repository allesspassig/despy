...
class DesmosAPI:
                ...
                if (len(lines)>0 and len(lines[0])>0):
                    ...
                    if ('GET' in lines[0].upper() and len(entries)>0):
                        if ('longpoll' in lines[0]):
                            ...
                            s.send(bytearray('  var xhrlongpoll = new XMLHttpRequest();xhrlongpoll.open("GET", "longpoll");xhrlongpoll.send();xhrlongpoll.addEventListener("load", function(e) {s=xhrlongpoll.responseText;calculator.setExpression({id:s.substr(0,s.indexOf(":")),latex:s.substr(s.indexOf(":")+1)});xhrlongpoll.open("GET", "longpoll");xhrlongpoll.send();});\r\n', 'utf-8'))
    ...
    def asyncUpdateEqn(self, latex, name):
        def func():
            ...
            self.longpoll.send(bytearray(name+':'+DesmosAPI.makeBackslashesComeInSinglesAndOnlySingles(latex), 'utf-8'))
            self.longpoll.close()
            self.longpoll = None
        longpollThread = threading.Thread(target = func)
